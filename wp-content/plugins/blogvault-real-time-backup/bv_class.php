...
class BlogVault {
	...
	*/
	...
	function dropDynSyncTable() {
		global $wpdb;
		$table_name = $this->getDynSyncTableName();
	...
	function getBvDynamincEvents($limit = 0, $filter = "") {
		...
		$result = array();
		$table_name = $this->getDynSyncTableName();
		$query = " SELECT * FROM $table_name $filter LIMIT $limit ";
		$rows = $wpdb->get_results($query);
		$last_id = 0;
		foreach ($rows as $data) {
			$result[] = array('event_data' => maybe_unserialize($data->event_data), 'event_tag' => $data->event_tag, 'event_type' => $data->event_type, 'site_id' => $data->site_id, 'id' => $data->id);
		$last_id = $data->id;
		}
	...
	function createDynSyncTable() {
		...
		if (array_key_exists('usedbdelta', $_REQUEST)) {
			...
			dbDelta( $query );
			return $wpdb->get_var("DESCRIBE $table_name;");
		} else {
			return $wpdb->query($query);
		}
	}
	...
	function processApiRequest() {
		...
		case "deletedynsyncevents":
			$this->addStatus("status", $this->deleteBvDynamicEvents($_REQUEST['filter']));
			break;
		...
		case "getdynamicevents":
			...
			if ($isdynsyncactive == 'yes') {
				...
				$this->addStatus("status", $this->getBvDynamincEvents($_REQUEST['limit'], $_REQUEST['filter']));
			}
			break;
		case "setwoodyn":
